...
### Agrega en L04
1. **`ExecutionState`:** Dataclass que acompaña la ejecución interna de cada run (`run_id`, `session_id`, `subject_id`, `messages`, `iteration`, `max_iterations`, `last_observation`, `termination_reason`, `tool_calls_executed`, `started_at`).
...
8. **Validación de Casos F1 y F2:**
   ...
   - **Caso F2:** Usuario Carlos fija preferencia; en nueva sesión se recuerda. Usuario Laura entra y no recibe la preferencia de Carlos (con comprobación primaria en la frontera de `MemoryStore`).
