...
/**
 * Lesson02Architecture — CASE Academy
 * Módulo 07 · Model Context Protocol — Lección 02 (c20)
 * Formato: Living Technical Documentation
 */
@Component({
  selector: 'app-lesson-02-architecture',
  standalone: true,
  imports: [
    CommonModule,
  ...
  templateUrl: './lesson-02-architecture.html',
  styleUrls: ['./lesson-02-architecture.css']
})
export class Lesson02Architecture implements OnInit {
  private courseService = inject(CourseService);
  private userProgressService = inject(UserProgressService);

  readonly doc = LESSON_02_DOCUMENT;
  
  readonly lesson: Lesson | undefined = this.courseService.getLessonByPath('/academy/modules/m07-mcp/lesson-02-architecture');
  readonly module: AcademyModule | undefined = this.courseService.getModuleById('m7');
  readonly adjacentLessons = this.courseService.getAdjacentLessons('c20');
